...
export function RightSidebarShell({
  ...
  const isCompactSurface = isReadingSurface || surfaceMode === 'map'

  const visibleTabs = useMemo<TopicWorkbenchTab[]>(
    () => (isReadingSurface ? ['assistant'] : ['assistant', 'research']),
    [isReadingSurface],
  )
  ...
  const emptyStateQuestions = isReadingSurface ? [] : suggestedQuestions.slice(0, 1)
  const emptyStateStarterPrompt = isReadingSurface ? '' : starterPrompt
  const emptyStateBrief = isReadingSurface ? null : research.brief
  const hasResearchIntel = hasMeaningfulResearchIntel(research.brief)
  const hasAssistantIntake = !isReadingSurface && Boolean(
    currentReadingEntry ||
  ...
  const revealReadingPanels = !isReadingSurface
    ? true
    : Boolean(
        currentThread.messages.length > 0 ||
          question.trim() ||
          agentBrief.trim() ||
          activeWorkbenchMaterials.length > 0 ||
          selectedEvidence,
      )

  useEffect(() => {
    if (researchViews.includes(researchView)) return
    setResearchView('search')
  }, [researchView, researchViews, setResearchView])

  useEffect(() => {
    if (!isReadingSurface || activeTab === 'assistant') return
    setActiveTab('assistant')
  }, [activeTab, isReadingSurface, setActiveTab])
  ...
  }, [topicId, research.durationDays, setOpen, setActiveTab, t])
  ...
  // Combined lifecycle effect: topic change, research brief prop, default drawer, URL params
  useEffect(() => {
    if (typeof window === 'undefined') return
    ...
    if (!hasPersistedWorkbenchState && window.innerWidth >= TOPIC_WORKBENCH_AUTO_OPEN_BREAKPOINT) {
      setOpen(true)
    }

    const requestedWorkbench = searchParams.get('workbench')
    ...
    const requestedResearchSurface =
      requestedWorkbench === 'research' || requestedFocus === 'research' || Boolean(requestedResearchView)
    const shouldOpenResearch = !isReadingSurface && requestedResearchSurface
    const shouldOpenAssistant =
      (requestedWorkbench === 'assistant' && !shouldOpenResearch) ||
      (isReadingSurface && requestedResearchSurface)
  ...
  }, [currentThread.messages.length, question, activeTab, visibleTabs, setActiveTab, setHistoryOpen, open, historyOpen, topicNotes.length])
  ...
  const saveNotebookEntry = useCallback(
    (entry: FavoriteExcerpt) => {
      addFavorite(entry)
      setOpen(true)
      // notes tab已移除，不再切换tab，保持当前assistant界面
    },
    [addFavorite, setOpen],
  )
  ...
  const researchStatusCards = !isReadingSurface ? (
    <>
      <ResearchSessionCard
    ...
    <>
      {(!isReadingSurface &&
        (!isCompactSurface ||
          ...
          Boolean(implicitFocusPill) ||
          readingTrailPills.length > 0)) ||
      (isReadingSurface && contextPills.length > 0) ? (
        <ContextTray
          items={contextPills}
          implicitFocus={implicitFocusPill}
          readingTrail={readingTrailPills}
          suggestions={isReadingSurface ? [] : contextSuggestions}
          onAdd={(pill) =>
            setContextPills((current) =>
              current.some((item) => item.id === pill.id) ? current : [pill, ...current],
            )
          }
          onCaptureSelection={isReadingSurface || isCompactSurface ? undefined : captureSelectionPill}
  ...
  const renderTabContent = () => {
    if (isReadingSurface || activeTab === 'assistant') {
      return (
        <div className="space-y-2">
          ...
          {hasAssistantIntake && (
            <section
              className={`border-t border-black/8 px-0 pt-3 ${
          ...
          {isReadingSurface && revealReadingPanels && references.length > 0 ? (
            <ReferencesPanel
          ...
          {isReadingSurface &&
          revealReadingPanels &&
          (resources.length > 0 || Boolean(selectedEvidence)) ? (
            <ResourcesPanel
              resources={resources}
    ...
    if (!isReadingSurface && activeTab === 'research') {
      return (
        <section data-testid="workbench-research-panel" className="space-y-3">
          {researchStatusCards}
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-[12px] leading-5 text-black/56">
                  {isReadingSurface
                    ? t(
                        'workbench.readingWorkspaceSummary',
                        'Keep the node reading surface focused: references and evidence stay here, while the main canvas switches between article and research views.',
                      )
                    : topicWorkbenchText(
