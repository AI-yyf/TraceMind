...
<p align="center">
  <strong>논문을 많이 모으는 것보다, 한 연구 분야의 맥락과 분기와 판단 근거를 분명하게 보게 해주는 AI 개인 연구 워크벤치.</strong>
</p>
...
TraceMind는 AI 개인 연구 워크벤치입니다. 이 프로젝트가 겨냥하는 순간은 다음과 같습니다.

"아직 논문을 못 찾았다"가 아니라,
"논문은 많이 찾았지만 이 방향이 실제로 어떻게 전개되는지 아직 명확히 설명할 수 없다"는 순간입니다.

TraceMind는 연구를 채팅 기록, 북마크, 흩어진 요약의 더미로 다루지 않습니다. 대신,

- 논문을 재사용 가능한 근거로
- 근거를 연구 노드로
- 노드를 근거 있는 판단으로
- 판단을 맥락을 잃지 않는 다음 질문으로

바꾸어 갑니다.

목표는 더 많은 텍스트를 생산하는 것이 아닙니다. 연구 방향을 읽을 수 있게 만드는 것입니다.

## 프로젝트 소개

TraceMind는 사용자가 실제로 마주하는 다섯 개의 핵심 표면으로 이해하는 것이 가장 쉽습니다.

| 표면 | 역할 | 사용자가 빠르게 이해해야 하는 것 |
| --- | --- | --- |
| 주제 페이지 | 한 연구 방향의 현재 상태 파악 | 어떤 stage가 있고, 어떤 node가 중요하며, 어떤 논문이 주선을 이루는가 |
| 노드 페이지: Research View | 노드에 빠르게 진입 | 이 노드가 무엇을 다루고, 어떤 근거가 중요하며, 어디서 합의와 분기가 생기는가 |
| 노드 페이지: Article View | 노드 심층 이해 | 노드 내부 여러 논문이 어떻게 연결되고, 긴 서술이 어떤 근거로 뒷받침되는가 |
| Workbench | 맥락 있는 후속 질문 | 현재 판단을 검증하고 분기를 비교하며 배경을 다시 설명하지 않고 질문을 이어가는 것 |
| 모델 센터 | 자신의 AI 스택 구성 | provider, model, base URL, API key, 작업별 라우팅을 직접 설정하는 것 |

한 문장으로 요약하면,

> TraceMind는 "논문 리스트 위에 채팅창을 얹은 도구"가 아니라 "연구 구조를 자라게 하는 도구"입니다.

## 주제 페이지: 먼저 방향을 보이게 만들기

주제 페이지는 연구 방향의 현재 위치를 파악하는 핵심 표면입니다. 여기서 빠르게 답해야 하는 질문은 이것입니다.

> "이 연구 방향은 지금 어디까지 왔고, 무엇이 실제 주선인가?"

TraceMind의 주제 페이지는 일반적인 프로젝트 보드처럼 보이면 안 됩니다. 주제를 만들자마자 가짜 `research planning` stage를 넣지도 않습니다. 주제는 가볍게 시작하고, 실제 연구 재료가 들어온 뒤에만 stage, node, judgment가 자라야 합니다.

### 주제 페이지에서 보이는 것

- 현재 실제 stage 수, 연구 node 수, 핵심 논문 수, 근거 수를 한눈에 보여주는 연구 진행 개요
- 논문 발견, 선별, 노드 합성, 시간창 누적에서 자라는 실제 stage timeline
- 주선, 분기, 합류 지점을 한 화면에서 보여주는 stage - node graph
- 각 stage당 최대 10개의 가시 node 카드만 보여 주어 복잡해져도 읽기 쉬운 구조
- 긴 목록에 묻히지 않도록 위로 끌어올린 key paper
- 중요한 node로 즉시 진입할 수 있는 빠른 입구
- 아직 정리되지 않은 재료도 보이게 해주는 pending 영역
- 현재 topic 맥락에서 바로 후속 질문을 이어갈 수 있는 오른쪽 workbench

### 좋은 주제 페이지가 30초 안에 알려줘야 하는 것

- 이 topic은 아직 탐색 단계인가, 아니면 꽤 구조화되었는가
- 지금 이 분야를 가장 잘 설명하는 stage는 무엇인가
- 계속 추적해야 할 branch는 어디인가
- 어떤 node가 설명의 중심을 맡는가
...
노드는 단일 논문 페이지가 아닙니다. 노드는 topic 안의 "구조화된 이해 단위"입니다. 방법 계열, 기술적 논쟁, 병목, 메커니즘, 한계, 전환점 등을 담을 수 있습니다.

그래서 노드 페이지는 본질적으로 두 가지 일을 해야 하고, TraceMind는 이를 dual view로 분리합니다.

| View | 목적 | 적합한 순간 |
| --- | --- | --- |
| Research View | 구조를 빠르게 파악 | 먼저 전체 윤곽을 짧은 시간에 보고 싶을 때 |
| Article View | 깊게 읽어 내기 | 이 노드가 중요하다고 판단한 뒤 논문군을 하나의 서사로 읽고 싶을 때 |

### Research View: 빠른 이해의 입구

Research View는 블로그 글보다 연구 브리프에 가깝습니다. 이상적인 감각은 다음과 같습니다.

> "연구 보조가 이 노드를 먼저 읽고, 가장 빨리 진지하게 이해할 수 있는 형태로 정리해 두었다."

Research View가 강조하는 것은 다음과 같습니다.
...
문장량보다 구조와 시각성을 우선해, 짧은 시간 안에 주선을 회복할 수 있도록 설계됩니다.

### Article View: 원문을 다 다시 열지 않아도 깊게 이해하게 해주기

Article View는 노드의 장문 읽기 레이어입니다. 원논문을 영원히 대체하려는 것이 아닙니다. 목적은 주선을 복구하기 위해 수많은 PDF를 즉시 다시 열어야 하는 상황을 줄이는 것입니다.

이를 위해 Article View는 다음을 제공합니다.

- 흩어진 요약이 아니라 연속된 노드 기사
- 논문과 근거 객체에 연결된 inline reference
- 가능할 때 그림, 표, 수식을 실제 서사 안으로 끌어오는 구성
- 같은 노드 안의 여러 논문을 하나의 이해선으로 묶는 통합
- 먼저 안정적인 기사 표면을 보여 주고, 이후 더 깊은 합성을 점진적으로 강화하는 흐름

TraceMind의 중요한 판단 중 하나는 여기 있습니다. 사용자는 어떤 원논문을 다시 정독할지 결정하기 전에, 먼저 "이 노드의 논문 묶음이 전체적으로 무엇을 말하는가"를 깊게 이해할 수 있어야 합니다.

## Workbench: 연구 도중 언제든 질문하기

연구 방향에 대한 이해는 한 번의 읽기로 끝나지 않습니다. 그래서 TraceMind에는 workbench가 필요합니다.
...
이것은 범용 채팅창이 아닙니다. 역할은 "맥락을 유지한 후속 질문"입니다. 좋은 질문은 보통 다음과 같습니다.
...
중요한 것은 "대화가 되는가"가 아니라 "topic이나 node의 맥락을 이어받아 질문을 계속할 수 있는가"입니다.

## 모델과 API: 나만의 스택을 연결하기

TraceMind는 사용자가 자기 모델 구성을 직접 통제하는 방식을 전제로 설계되었습니다.
...
그래서 TraceMind는 다음 환경에서 유연하게 동작할 수 있습니다.
...
중요한 점은 TraceMind가 `question`에서 `answer`로 바로 점프하려 하지 않는다는 것입니다. 지키려는 것은 그 중간 구조입니다.
...
2. 최소 하나의 language model을 설정하고, PDF/그림/표/수식 처리를 강화하려면 multimodal model도 추가합니다.
3. 몇 주 이상 정말 이해하고 싶은 실제 topic을 만듭니다.
4. 논문 발견을 실행한 뒤 후보를 그대로 다 넣지 말고 선별합니다.
5. topic 페이지로 돌아가 stage, node, key paper가 의미를 가지기 시작했는지 확인합니다.
6. node는 Research View부터 들어가고, 필요하면 Article View로 이동해 깊게 읽습니다.
7. workbench에서 현재 판단을 의심하고 어디가 약한지 파고듭니다.
...
| 노트 앱 / 위키 | 유연한 수동 정리 | 수동 메모만이 아니라 문헌을 연구 객체로 바꾸는 흐름을 제공한다 |
...
1. 논문이 아니라 방향에서 시작합니다.
   예: "멀티모달 agent planning에서 무엇이 바뀌고 있는가"
2. 후보 풀을 만든 뒤에는 과감하게 거절합니다.
   주변 논문을 너무 많이 넣으면 topic은 절대 맑아지지 않습니다.
3. 하위 문제를 중심으로 node가 자연스럽게 자라게 합니다.
   좋은 node는 방법 계열, 병목, 평가 논쟁, 기술 전환을 중심으로 형성됩니다.
4. 깊게 읽기 전에 topic page를 봅니다.
   어느 node를 먼저 볼지 topic page가 알려줘야 합니다.
5. 먼저 Research View, 그다음 Article View로 갑니다.
   먼저 구조를 회복하고, 그다음 깊이 읽습니다.
6. Article View를 이용해 원문을 전부 다시 열기 전에 node 전체를 이해합니다.
7. Workbench에서 약점을 공격합니다.
   무엇이 과장되었는지, 무엇이 부족한지, 무엇이 판단을 바꿀지를 묻습니다.
8. node가 읽히는 상태가 되었을 때 notes나 report material을 내보냅니다.
...
연구 제품은 그럴듯한 출력만 최적화하면 금방 잡음이 됩니다. 그래서 이런 원칙이 필요합니다.

## 시작점

하나의 연구 업데이트만으로는 전체 방향을 거의 볼 수 없습니다. 특히 오늘날 AI 연구는 속도가 빠르고, 유행을 따라가는 압력이 강하며, 먼저 반응한 사람이 보상을 받기 쉽습니다.

그것은 정보 파악에는 도움이 되지만, 진짜 이해에는 충분하지 않습니다. 모두가 최신성만 쫓으면,

- 무엇이 실제로 축적되고 있는지
- 무엇이 단지 재포장인지
- 어떤 분기가 아직 해결되지 않았는지
- 어떤 근거가 실제로 흐름을 바꾸었는지

를 꾸준히 추적하는 사람이 줄어들기 때문입니다.
