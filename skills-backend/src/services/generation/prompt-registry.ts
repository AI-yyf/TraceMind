...
const BUILT_IN_PRODUCT_COPIES: Record<ProductCopyId, ProductCopyDefinition> = {
  ...
  [PRODUCT_COPY_IDS.CREATE_LANGUAGE_OPTION_ZH]: defineCopy(PRODUCT_COPY_IDS.CREATE_LANGUAGE_OPTION_ZH, 'create', '创建主题语言选项·中文', '中文源语言选项说明。', 256, '以简体中文创建轻量主题外壳，后续由真实研究材料长出阶段。', 'Create a lightweight topic shell from Simplified Chinese, then let stages grow from real research material.', true),
  [PRODUCT_COPY_IDS.CREATE_LANGUAGE_OPTION_EN]: defineCopy(PRODUCT_COPY_IDS.CREATE_LANGUAGE_OPTION_EN, 'create', '创建主题语言选项·英文', '英文源语言选项说明。', 257, '以英文创建轻量主题外壳，后续由证据与节点长出阶段。', 'Use English as the source language, create a lightweight topic shell, and grow stages from evidence.', true),
...
const BUILT_IN_PROMPTS: Record<PromptTemplateId, PromptTemplateDefinition> = {
  [PROMPT_TEMPLATE_IDS.TOPIC_PREVIEW]: {
    ...
    family: 'topic',
    title: '主题创建预览',
    description: '用于创建主题时生成主题名、摘要、关键词与研究焦点。',
    slot: 'language',
    order: 5,
